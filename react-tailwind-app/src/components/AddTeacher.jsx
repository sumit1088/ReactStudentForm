...
const AddTeacher = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate centerId, schoolId, and required fields for teachers
        if (!centerId || !schoolId) {
            alert("Please select a center and a school");
            return;
        }

        // Check if teachers array is not empty and has the required fields
        if (!teachers || teachers.length === 0 || teachers.some(t => !t.name || !/^\d{10,11}$/.test(t.contactNo1) || !/^\d{10,11}$/.test(t.contactNo2) || !/\S+@\S+\.\S+/.test(t.email))) {
            alert("Please fill all required teacher fields correctly");
            return;
        }

        // Map teachers to add centerId and schoolId
        const formattedTeachers = teachers.map(t => ({
            id: 0,  // Assuming ID is auto-generated by backend
            name: t.name,
            contactNo1: t.contactNo1,
            contactNo2: t.contactNo2,
            email: t.email,
        ...
        try {
            ...
            const response = await fetch('http://192.168.1.28:8075/api/Teachers/Bulk', {
                ...
                body: JSON.stringify(formattedTeachers), // Send the formatted teacher array
            });
